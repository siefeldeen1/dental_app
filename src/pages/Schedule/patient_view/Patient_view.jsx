    ...
    <div className='table_big_dev'>
      <div className='table_header_group'>
            <div className='search'>
                <AiOutlineSearch/>
                <input className='search_input' value={search} onChange={(e)=>{setsearch(e.target.value)}} placeholder='search' type="text" />
            </div> 
        </div>  

        <div className='bigger_dev_table_cont'>
            <div className='table_header_title'>Daily patients 
               {/* <span className='span_table_header_title'>(02/10/2023)</span> */}
            </div>
            <hr className='hr_table_header_title' />
                <table>
                      ...
                      { patients.filter(ele=> ele["name"].includes(search)).map((e,i)=>{
                        return(
                      ...
                      { patients.filter(ele=> ele["added_date"]?.includes(added_date)).map((e,i)=>{
                        // return(
                        // //   <tr key={i} className="patients_table">
                        // //       <td>{localStorage.getItem("clinic_id")}</td>
                        // //       <td>{e["patient_id"]}</td>
